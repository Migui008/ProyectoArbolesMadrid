<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="styles.css">
    <link rel="icon" type="image/png" href="icono.png">
    <?php
    session_start();
    require_once("functions.php");
    $idParque = 13;
    $nombreParque = "Jardines Plaza de Oriente - Jardín del Cabo Noval-Jardín de Lepanto";
    if (!isset($_SESSION[$nombreParque])) {
        getInfoParque($idParque);
    }
    $infoParque = $_SESSION[$nombreParque];
    $bus = explode(" ",$infoParque["transporte_bus"]);
    $metro = explode(" ",$infoParque["transporte_metro"]);
    $renfe = explode(" ",$infoParque["transporte_renfe"]);
    ?>
  	<title><?= $nombreParque ?></title>
</head>
<body>
    <?php require_once("header.php"); ?>
  <div class="mainBody">
    <div class="mainArticleInfo">
        <div class="content">
            <div class="languageSwitcher">
                <form class="langForm" id="langForm" action="" method="post">
                    <input type="hidden" name="lang" value="es">
                    <button type="submit"><img src="data:image/png;base64," alt="Español"></button>
                </form>
                <form class="langForm" id="langForm" action="" method="post">
                    <input type="hidden" name="lang" value="en">
                    <button type="submit"><img src="data:image/png;base64," alt="English"></button>
                </form>
            </div>
            <?php
                echo "<h1>$nombreParque</h1>";
                if (!isset($_POST['lang']) || $_POST['lang'] == 'es') {
                    foreach ($_SESSION[$nombreParque]['contenido'] as $contenido) {
                        echo "<h2 class='contentTitle'>".$contenido['titulo']."</h2>";
                        echo "<p class='contentText'>".$contenido['texto']."</p>";
                        echo "<br>";
                    }
                } elseif ($_POST['lang'] == 'en') {
                    foreach ($_SESSION[$nombreParque]['contenido'] as $contenido) {
                        echo "<h2 class='contentTitle'>".$contenido['titulo_en']."</h2>";
                        echo "<p class='contentText'>".$contenido['texto_en']."</p>";
                        echo "<br>";
                    }
                }
            ?>
        </div>
        <div class="transportePublico">
            <?php
            if (!empty($bus)) {
                echo "<h4>Bus</h4>";
                echo "<p>".implode(" ", $bus)."</p>";
            }
            if (!empty($metro)) {
                echo "<h4>Metro</h4>";
                echo "<p>".implode(" ", $metro)."</p>";
            }
            if (!empty($renfe)) {
                echo "<h4>Renfe</h4>";
                echo "<p>".implode(" ", $renfe)."</p>";
            }
            ?>
        </div>
    </div>
    <div class="sideContent">
        <?php
        $imagenParque = "data:image/png;base64,".$infoParque["imagen"];
        echo '<img class="imagenParque" id="imagenParque" src="' . $imagenParque . '" alt="Imagen">';
        ?>
        <div id="map" style="height: 400px; width: 100%;"></div>
        <?php
        if (!isset($_POST['lang']) || $_POST['lang'] == 'es') {
            echo "<h4>Árboles disponibles</h4>";
        } elseif ($_POST['lang'] == 'en') {
            echo "<h4>Available trees</h4>";
        }
        foreach ($infoParque['arboles_relacionados'] as $arbol) {
            $arbolN = $arbol["nombre"];
			echo "<a class='enlaceRelaciones' href='/$arbolN'>$arbolN</a><br>";
        }
      	if (!isset($_POST['lang']) || $_POST['lang'] == 'es') {
          	echo "<h4><b>Latitud: </b></h4>";
 			echo "<p>".$infoParque["latitud"]."</p>";
          
          	echo "<h4><b>Longitud: </b></h4>";
          	echo "<p>".$infoParque["longitud"]."</p>";
          
         	echo "<h4><b>Dirección: </b></h4>";
          	echo "<p>".$infoParque["direccion"]."</p>";
          	} elseif ($_POST['lang'] == 'en') {
          	echo "<h4><b>Latitude: </b></h4>";
 			echo "<p>".$infoParque["latitud"]."</p>";
          
          	echo "<h4><b>Longitude: </b></h4>";
          	echo "<p>".$infoParque["longitud"]."</p>";
          
         	echo "<h4><b>Address: </b></h4>";
          	echo "<p>".$infoParque["direccion"]."</p>";
        	}
        ?>
    </div>
  </div>

    <script>
    var latitud = parseFloat(<?= json_encode($infoParque["latitud"]) ?>);
    var longitud = parseFloat(<?= json_encode($infoParque["longitud"]) ?>);
    console.log(latitud);
    </script>
    <script src="functions.js"></script>
    <script async defer
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCLGJGJeXOzxATwdcn6gF7JYj2aseHiYT4&callback=initMap">
    </script>

</body>
</html>